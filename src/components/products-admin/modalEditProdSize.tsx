...
import { updateProductSize } from "@/services/productSizeService";

import { setFetched as setProductSizesFetched } from '@/redux/productSize/sliceProductSize';

import { useDispatch } from 'react-redux';

export default function ModalEditProdSize({ id }: { id?: string | null }) {
    const productsState = useSelector((state: RootState) => state.products);

    const productsSizeState = useSelector((state: RootState) => state.productSize);

    // Find the product-size to edit from the store (sizes holds all product-size items)
    const productSizePatch = productsSizeState.sizes.find((p: any) => (p._id ?? p.id) === id);

    
    const { isOpen, onOpenChange } = useModal('editProdSizeModal');
    ...
    const updateProdSize = async (closeModal: () => void) => {
        try {
            setIsLoading(true);
            const params = {
            ...
            console.debug('updateProdSize - sending params:', { id, params, productSizePatch });
            if (!id) throw new Error('ModalEditProdSize - id is required');
            await updateProductSize(id, params);
            ...
            console.error('Error actualizando producto talle:', err);
            try { alert((err as any)?.message || 'Error actualizando producto talle'); } catch(e) { /** noop */ }
        }
        finally {
            setIsLoading(false);
    ...
    useEffect(() => {
        if (isOpen && productSizePatch) {
            setFormData({
                id_product: productSizePatch.id_product || '',
                size: productSizePatch.size || '',
                stock: productSizePatch.stock || 0
            });
        }
    }, [isOpen, productSizePatch]);
            ...
            <>
              ...
              <ModalFooter className="flex flex-row gap-2 sm:gap-3">
                ...
                <Button 
                  isLoading={isLoading} 
                  color="primary" 
                  onPress={() => updateProdSize(onClose)}
                  className="text-sm sm:text-base"
                  size={isMobile ? "md" : "lg"}
                >
                    Editar Producto Talle
                </Button>
              </ModalFooter>
            </>
          )}
